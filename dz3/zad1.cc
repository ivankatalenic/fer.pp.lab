	...
	try {
		...
		std::string source{R"CLC(
			kernel void sum(
				...
				// Start of the scheduling part of the algorithm.
				// Divide the work in batches.
				// Each batch has threads_cnt items, so each thread is processing one item in the batch.
				
				// Process the last uneven batch separately, if exists
				if (work_per_thread % 2 == 1) {
					// Do the last batch
					const int start = (work_per_thread - 1) * threads_cnt + thread_id;
					C[start] = 0.0;
					for (int end = start + 1; end < work_size; end++) {
						C[start] += distance((double2)(X[start], Y[start]), (double2)(X[end], Y[end]));
					}
				...
				// First half of the rest
				for (int batch = 0; batch < work_per_thread / 2; batch++) {
					const int start = batch * threads_cnt + thread_id;
					C[start] = 0.0;
					for (int end = start + 1; end < work_size; end++) {
						C[start] += distance((double2)(X[start], Y[start]), (double2)(X[end], Y[end]));
					}
				...
				// Second half of the rest
				for (int batch = work_per_thread / 2; batch < 2 * (work_per_thread / 2); batch++) {
					const int start = (batch + 1) * threads_cnt - thread_id - 1;
					C[start] = 0.0;
					for (int end = start + 1; end < work_size; end++) {
						C[start] += distance((double2)(X[start], Y[start]), (double2)(X[end], Y[end]));
					}
